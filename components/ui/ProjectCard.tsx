        ...
        <div className="flex flex-wrap gap-2">
          {project.tags.map((tag) => {
            ...
            return (
              <span
                key={tag}
                className={`border border-portfolio-rule group-hover:border-[#a8a8a3] transition-colors duration-[400ms] ease-in-out rounded-full px-3 py-[5px] text-caption font-mono text-portfolio-muted uppercase tracking-wider whitespace-nowrap${isFilled ? " bg-portfolio-rule/60" : ""}`}
              >
                {tag}
              </span>
            );
